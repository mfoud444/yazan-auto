...
<template>
  <div class="dark:bg-gray-950 dark:text-white flex py-24 w-full min-h-[65vh]  rounded-lg gap-22 relative">
    <!-- Vanta Container for visual effects -->
    <div ref="vantaContainer" class=" top-0 absolute w-full h-full z-0"></div>
    
    <!-- Content overlaid on the Vanta container -->
    <div class="flex flex-col md:gap-8 z-46 justify-end items-center relative h-full">
      ...
      <div class="flex flex-col items-center justify-end w-full h-full">
        ...
        <!-- Button -->
        <button
          class="btn btn-primary font-bold text-2xl w-56 mt-4"
          @click="goService"
        >
          {{ t('app.start') }}
        </button>
